...
original_author="D3fus"
original_name="testing"
original_urlname="testing"
original_description="Awesome testing created by D3fus" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/testing "${name}"
...
then
    ...
    echo "then run 'testing' or 'python -m testing'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'testing' or 'python -m testing' or 'poetry run testing'"
fi
